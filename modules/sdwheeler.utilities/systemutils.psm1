...
function kb {
  ...
  foreach ($k in $kb) {
    $k = $k -replace '[a-zA-Z]', ''
    $article = Invoke-RestMethod "https://support.microsoft.com/app/content/api/content/help/en-us/$k"
    $article | Select-Object @{n = 'id'; e = { $_.details.id } }, @{n = 'title'; e = { $_.details.title } }
  }
}
#-------------------------------------------------------
