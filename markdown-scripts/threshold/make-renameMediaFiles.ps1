...
<#
    ...
    .DESCRIPTION
    This script scans the current folder (recursively) for all non-MD files and creates the CSV file. The CSV
    ...
    renameMediaFiles.csv in the current folder.

    .EXAMPLE
    PS > cd "<repo-location>\networking"
    PS > <script-repository>\make-renameMediaFiles.ps1

    .EXAMPLE
    PS > cd "<repo-location>\networking"
    PS > <script-repository>\make-renameMediaFiles.ps1 c:\temp\renameMediaFiles.csv

#>

[CmdletBinding()]
param([string]$csvfile = '.\renameMediaFiles.csv')
...
Get-ChildItem -Path .\media -Recurse |
