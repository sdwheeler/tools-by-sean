...
do {
    $tokensFound  = get-childitem -Path $documentsfolder -Filter '*.md' -recurse |
        ...
        ForEach-Object {
            ...
            $_.matches | % {
                @{ document=$document
                   token=$_.groups[0].value; tokenFile = $(Join-Path -Path $documentsRoot -ChildPath $($_.groups[1].value.Replace('/','\')))
                }
            }
    ...
    ForEach-Object {
        ...
        $_.matches | % {
            @{ document=$document
               ...
               tokenFile = $(Join-Path -Path $(([System.IO.Path]::GetDirectoryName($document))) -ChildPath $($_.groups[1].value))
            }
        }
    }
